...
class Fire(tf.keras.Model):
    def __init__(
        self, inplanes, squeeze_planes, expand1x1_planes, expand3x3_planes, name=None
    ):
        super(Fire, self).__init__(name="%s/fire" % name)
        self.inplanes = inplanes
        self.squeeze = tf.keras.layers.Conv2D(
            squeeze_planes,
            input_shape=(inplanes,),
            kernel_size=1,
            strides=(1, 1),
            padding="VALID",
            activation="relu",
        ...
        self.net = tf.keras.models.Sequential(
            [
                ...
                tf.keras.layers.Conv2D(
                    ...
                    padding="VALID",
                    activation="relu",
                    name="classifier/layer1",
                ),
                tf.keras.layers.AveragePooling2D(
                    pool_size=13, strides=13, padding="VALID", name="classifier/layer3"
                ),
            ]
        )
